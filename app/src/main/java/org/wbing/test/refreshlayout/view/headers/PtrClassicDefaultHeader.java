...
public class PtrClassicDefaultHeader extends LinearLayout implements ILoadingLayout {
    ...
    public PtrClassicDefaultHeader(Context context) {
        ...
        setLayoutParams(new RefreshLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        initViews(null);
    }
    ...
    public void onUIPositionChange(RefreshLayout frame, boolean isUnderTouch, byte status, RefreshLayoutIndicator ptrIndicator) {
        ...
        final int mOffsetToRefresh = frame.getRefreshIndicator().getOffsetToRefresh();
        final int currentPos = ptrIndicator.getCurrentPosY();
        final int lastPos = ptrIndicator.getLastPosY();

        if (currentPos < mOffsetToRefresh && lastPos >= mOffsetToRefresh) {
            if (isUnderTouch && status == RefreshLayout.STATUS_PREPARE) {
    ...
    private void crossRotateLineFromTopUnderTouch(RefreshLayout frame) {
        mTitleTextView.setVisibility(VISIBLE);
        mTitleTextView.setText(R.string.cube_ptr_release_to_refresh);
    }

    private void crossRotateLineFromBottomUnderTouch(RefreshLayout frame) {
        mTitleTextView.setVisibility(VISIBLE);
        mTitleTextView.setText(getResources().getString(R.string.cube_ptr_pull_down_to_refresh));
    }
